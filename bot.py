...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text_lower in ['помощь', 'help']:
        await update.message.reply_text(
            "📋 СПРАВКА ПО КОМАНДАМ\n\n"
            "🏢 НАЧАЛО:\n"
            "• Нажмите кнопки выбора клуба\n\n"
            "💰 ВВОД ДАННЫХ:\n"
            "• НАЛ → вставить данные\n"
            "• БЕЗНАЛ → вставить данные\n"
            "• ГОТОВО → указать дату (3,10)\n\n"
            "📊 ОТЧЁТЫ:\n"
            "• ОТЧЁТ → клуб → период\n"
            "• ВЫПЛАТЫ → код + период (Д7 3,10)\n\n"
            "📝 РЕДАКТИРОВАНИЕ:\n"
            "• СПИСОК → дата (3,10)\n"
            ...
            "• ЭКСПОРТ → клуб → период\n\n"
            "🔧 ДОПОЛНИТЕЛЬНО:\n"
            "• ОБНУЛИТЬ → удалить все данные (пин)\n"
            "• ЗАВЕРШИТЬ → выйти (потребуется пин)\n\n"
            "📖 ФОРМАТЫ ДАТ:\n"
            "• 3,10 = 03.10.2025\n"
            "• 30,10 = 30.10.2025\n"
            "• 3,10-5,11 = период\n\n"
